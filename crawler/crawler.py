        ...
        with console.status("[bold blue]Loading existing assessment data...[/bold blue]"):
            try:
                ...
                for assessment in existing_assessments:
                    ...
                    # Ensure solution_type exists for backward compatibility
                    if 'solution_type' not in assessment:
                        assessment['solution_type'] = 'individual'
                        updated = True
                
                # Persist back if we added missing fields
                if updated:
                    with open(OUTPUT_FILE, 'w', encoding='utf-8') as out_f:
                        json.dump(existing_assessments, out_f, indent=2, ensure_ascii=False)
                    console.print(f"[green]✓[/green] Added missing 'solution_type' to existing assessments and saved to {OUTPUT_FILE}")
...
def extract_assessment_links(soup, section_type):
    ...
    for link in assessment_links:
        ...
        assessment = {
            ...
            'test_types': [],
            'description': None,
            # Tag the solution type so we can filter later (individual or prepackaged)
            'solution_type': 'individual' if section_type == 'individual' else 'prepackaged'
        }
